...
export type LiveFactorMeta = Partial<Record<FactorId, { source: string; updatedAt: Date; reason?: string }>>
...
// All 16 factors' weights sum to 1.00. Convention throughout: a higher `val`
...
export const FACTORS: Factor[] = [
  ...
  { id: 'china',           label: 'China demand',                min: 1,   max: 10,  unit: '/10',    weight: 0.12,  icon: '🇨🇳', val: 7,    live: true },
  { id: 'deficit',         label: 'India deficit (L MT)',        min: 2,   max: 9,   unit: ' L MT',  weight: 0.12,  icon: '📦', step: 0.5, val: 5.5, live: true },
  { id: 'monsoon',         label: 'Monsoon disruption',           min: 1,   max: 10,  unit: '/10',    weight: 0.10,  icon: '🌧️', val: 6 },
  { id: 'seasia',          label: 'SE Asia supply',               min: 1,   max: 10,  unit: '/10',    weight: 0.08,  icon: '🌏', val: 6,    live: true },
  ...
  { id: 'crude',           label: 'Brent crude',                  min: 55,  max: 115, unit: ' $/bbl', weight: 0.05,  icon: '🛢️', val: 82,   live: true },
  { id: 'tokyo',           label: 'Japan demand (Tokyo)',         min: 1,   max: 10,  unit: '/10',    weight: 0.03,  icon: '🗼', val: 6,    live: true },
  { id: 'govPolicy',       label: 'Export policy tightness',      min: 1,   max: 10,  unit: '/10',    weight: 0.04,  icon: '📜', val: 6 },
  ...
  { id: 'autoSales',       label: 'Global auto sales & EV mix',   min: 1,   max: 10,  unit: '/10',    weight: 0.03,  icon: '🚙', val: 6,    live: true },
  { id: 'inr',             label: 'INR/USD rate',                 min: 80,  max: 93,  unit: ' ₹/$',   weight: 0.02,  icon: '💱', val: 84.5, live: true },
  { id: 'shipping',        label: 'Shipping index',                min: 1,   max: 10,  unit: '/10',    weight: 0.015, icon: '🚢', val: 6,    live: true },
...
export function computePrediction(factors: Factor[]) {
  ...
  const crude           = (f.crude - 75)           * 0.175
  const tokyo           = (f.tokyo - 5)            * 0.69
  const govPolicy       = (f.govPolicy - 5)        * 0.92
  ...
  const total = base + tireDemand + china + deficit + monsoon + seasia + exchangeStock +
    syntheticRubber + crude + tokyo + govPolicy + producerFx + laborDisruption + autoSales + inr +
  ...
  return {
    ...
    contribs: [
      ...
      { id: 'tokyo' as FactorId,           name: 'Japan/Tokyo demand', val: Math.round(tokyo) },
